...
The scaffolder ships a single opinionated workspace shape. The compiler
([`@keyyard/bedrock-build`](/docs/reference/cli-build)) is built around
this layout, and the [`create` generators](/docs/reference/cli-create-generators)
write into it. Straying from it means hand-editing config to keep things
wired up.
...
```
<project>/
  config.json                 ← compiler entry point
  package.json
  tsconfig.json
  .gitignore
  src/
...
output, and how to deploy. The `bedrock-cli` keys (entry, out, deploy)
live under the `bedrock-cli` namespace, while pack source paths live in
`packs.behaviorPack` and `packs.resourcePack`. Older projects with a
`bedrock.config.json` are still read as a fallback, so they keep working.
Full reference: [Config Schema](/docs/reference/config-schema).

### `src/`

Your TypeScript or JavaScript. The entry file is whatever
`bedrock-cli.entry` points at in `config.json` (default: `src/main.ts`,
though `src/main.js` works too). Everything the entry file imports is
bundled into a single output file by esbuild.
...
The subfolder names inside each pack follow Bedrock's canonical layout
(`entities/`, `items/`, `textures/`, `models/entity/`, etc.) and there
are some footguns: BP uses `entities/` (plural) but RP uses `entity/`
(singular), `models/entity/` and `textures/entity/` are also singular.
Don't try to remember it. Run [`npm run folders`](/docs/reference/cli-build#folders)
and tick the ones you need from a checklist.
